...
class ActivityScreen extends React.Component {
   ...
   async componentDidMount() {
      ...
      activity.timeslots.forEach((timeslot) => {
         const childParticipants = JSON.parse(timeslot.extendedProperties.shared.children);
         const parentParticipants = JSON.parse(timeslot.extendedProperties.shared.parents);
      ...
      activity.children = activity.children.sort((a, b) => `${a.given_name} ${a.family_name}` - `${b.given_name} ${b.family_name}`);
      activity.parents = activity.parents.sort((a, b) => `${a.given_name} ${a.family_name}` - `${b.given_name} ${b.family_name}`);
      ...
      return list.map((profile, index) => (
         <li key={index} style={{ display: 'block' }}>
            <div className='row' style={{ margin: '1rem 0' }}>
               <Avatar route={type === 'parents' ? `/profiles/groupmember/children/${profile.child_id}` : `/profiles/${profile.user_id}/info`} className={classes.avatar} thumbnail={path(profile, ['image', 'path'])} disabled={profile.suspended} />
               <div role='button' tabIndex={-42} className='participantsText' onClick={() => this.handleRedirect(profile.suspended, profile.child_id)}>
   ...
   renderParticipants = (type) => {
      ...
      const showParticipants = type === 'volunteers' ? showVolunteers : showChildren;
      const stateProperty = type === 'volunteers' ? 'showVolunteers' : 'showChildren';
      const profiles = type === 'volunteers' ? parents : children;
      return (
         <React.Fragment>
            <div className='row no-gutters' style={rowStyle}>
               <div className='col-1-10'>{type === 'volunteers' ? <i className='fas fa-user-friends' /> : <img src={Images.babyFace} alt='map marker icon' className='activityInfoImage' />}</div>
               ...
               <div className='col-1-10'>
                  <button
                     ...
                     <i className={showParticipants ? 'fas fa-chevron-up activityInfoIcon' : 'fas fa-chevron-down activityInfoIcon'} />
                  </button>
               </div>
            </div>
            <div className='row no-gutters' style={showParticipants ? rowStyle : { display: 'none' }}>
               <div className='col-1-10' />
   ...
   render() {
      ...
      const { activity, fetchedActivityData, userCanEdit, action, confirmDialogIsOpen, optionsModalIsOpen, pendingRequest } = this.state;
      const texts = Texts[language].activityScreen;
      const options = [
         {
            label: texts.delete,
      ...
      const confirmDialogTitle = action === 'delete' ? texts.deleteDialogTitle : texts.exportDialogTitle;
      const rowStyle = { minHeight: '5rem' };
      return fetchedActivityData ? (
         <React.Fragment>
            {pendingRequest && <LoadingSpinner />}
            <div id='activityContainer'>
               <ConfirmDialog title={confirmDialogTitle} isOpen={confirmDialogIsOpen} handleClose={this.handleConfirmDialogClose} />
               <OptionsModal isOpen={optionsModalIsOpen} handleClose={this.handleOptionsClose} options={options} />
               <div id='activityHeaderContainer' className='row no-gutters'>
                  ...
                  <div class='modal'>
                     <div class='modal_content'>
                        ...
                        <span class='close' style={{ float: 'none' }}>
                  ...
                  <div className='col-1-10'>
                     {userCanEdit ? (
                        <button type='button' className='transparentButton center' onClick={this.handleEdit}>
                  ...
                  <div className='col-1-10'>
                     {userCanEdit ? (
                        <button type='button' className='transparentButton center' onClick={this.handleNotification}>
                  ...
                  <div className='col-1-10'>
                     {userCanEdit ? (
                        <button type='button' className='transparentButton center' onClick={this.handleOptions}>
